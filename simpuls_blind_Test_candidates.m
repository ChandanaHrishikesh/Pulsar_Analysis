function [ampiez_cand_max freq_cand_max f0dot_cand_max lambda_fin_max RA_cand_max DEC_cand_max]=simpuls_blind_Test_candidates(ra,dec,mp_ra,mp_dec,ROI,T_start,T_stop,E_min,PEPOCH,spin_par,n_pos,Tp,dt,lambda_par,blind,CRThr,spin_down,frband,T_obs,lambda_points)
    ...
    size_time_photon=length(t_fotoni_mjd);
    param.size_time_photon=size_time_photon;
    h=0;
    ...
    %Tobs
    filename=sprintf('output_%dd_%d_%f_%s%s%s%s_%02d%02d.mat',Tp,n_pos,T_stop,ra(1),ra(2),ra(4),ra(5),dec(1),dec(2))
    filename2=sprintf('CAND_%dd_%d_%f_%s%s%s%s_%02d%02d.mat',Tp,n_pos,T_stop,ra(1),ra(2),ra(4),ra(5),dec(1),dec(2))
    ...
    for k = k_min:k_max % sky loop
        ...
        for kk = kk_min:kk_max 
            ...
            TimeSkyPoint1=toc(TimeSkyPoint0);
            param.TimeSkyPoint1=TimeSkyPoint1;
    ...
    %fotone(I rewrite the parameters with respect to the first arrival time of the photon)
            ...
            %if (spin_par(3) ~= 0)   
                ...
                delta_f0dot = 1/(2* T_obs*Tp*86400);
                delta_lambda = (delta_f0dot*2*dt)/2; % c'e' 2*dt perche' la frequenza massima e' 1/64 ma quella fisica, che ci interessa e' 1/32 (there is 2*dt because the maximum frequency is 1/64 but the physical one, which interests us, is 1/32)
                param.delta_lambda=delta_lambda;
                lambda_num=round((lambda_max-lambda_min)/delta_lambda);
                param.lambda_num=lambda_num;
                TimeNLambda=tic;
                for j=1:lambda_num
                    hh=hh+1;
                  ...
                  %  ('SPETTRO-AUTOCOR')
                    ...
                    % CR and candidate selection from here (To be completed
                    Stat=robstat(y,0.1);
                    ...
                    save(filename2,'CAND','param','-v7.3');
                end
                param.TimeNLambda=toc(TimeNLambda);
    ...
    save(filename,'CAND','ampiez_cand_max','freq_cand_max','f0dot_cand_max','lambda_fin_max','RA_cand_max','DEC_cand_max','epsilon','param');
    ...
    load (filename);
    %fprintf(fid1, 'Tp\tfrband(1)\tfrband(2)\tspin_down\tCRThr\tn_pos\tT_obs\tT_obs_simpuls\tlambda_points\tlambda_num\tTimeNLambda\tTimeSkyPoint1\n');
    fprintf(fid1, '%f\t%f\t%f\t%0.6e\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', Tp,frband(1),frband(2),spin_down,CRThr,n_pos,T_obs,param.T_obs,lambda_points,param.lambda_num,param.TimeNLambda,param.TimeSkyPoint1);
